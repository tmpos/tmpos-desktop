...
async function verificarLicenciaApp() {
  ...
  try {
    ...
    console.log('[Login] Resultado licencia:', { success: Boolean(result?.success), estado: result?.estado || result?.data?.estado || '' })
...
async function actualizarUsuariosDesdeApi() {
  // El equipo principal mantiene la copia SQLite sincronizada. Un navegador
  // de red no debe intentar modificar usuarios antes de poder autenticarse.
  if ((window as any).__isNetworkClient) return
