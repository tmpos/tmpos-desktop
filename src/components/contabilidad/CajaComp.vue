...
function perteneceAlmacenActual(factura) {
  ...
  return !Number(factura.almacen_id) || Number(factura.almacen_id) === Number(almacenStore.activeId)
}
...
async function cargarVentas() {
  ...
  try {
    ...
    abonosTurno.value = obtenerAbonosTurno(cuentasRes.success ? cuentasRes.data || [] : [], turnoActual.value.id)
    cobrosTallerTurno.value = obtenerCobrosTaller(tallerRes.success ? tallerRes.data || [] : [], turnoActual.value.id, turnoActual.value.created_at)
...
async function cobrarFacturaPendiente(factura) {
  ...
  try {
    ...
    bancosCobroPendiente.value = bancosRes.success ? (bancosRes.data || []).filter(b => !almacenStore.activeUid || !b.almacen_uid || String(b.almacen_uid) === String(almacenStore.activeUid)) : []
...
function obtenerAbonosTurno(cuentas, turnoId, turnoInicio = turnoActual.value?.created_at) {
  const inicio = parseDbDate(turnoInicio)
  return cuentas.flatMap(cuenta => {
    let pagos = []
    try { pagos = Array.isArray(cuenta.pagos) ? cuenta.pagos : JSON.parse(cuenta.pagos || '[]') } catch { pagos = [] }
    return pagos.filter(p => {
      if (Number(p.turno_id || 0) > 0) return Number(p.turno_id) === Number(turnoId)
      // Compatibilidad con abonos creados antes de guardar el turno_id.
      const fechaPago = parseDbDate(p.created_at || `${p.fecha || ''} ${p.hora || ''}`)
      return inicio > 0 && fechaPago >= inicio
    }).map((p, index) => ({
...
function obtenerCobrosTaller(ordenes, turnoId, turnoInicio) {
  ...
  return ordenes.flatMap(orden => {
    ...
    if (!cobros.length && String(orden.nombre || '').toUpperCase() === 'REPARACION EXPRESS' && String(orden.estado || '').toUpperCase() === 'COMPLETADO' && parseDbDate(orden.created_at) >= inicio) {
      cobros.push({ id: `legacy-${orden.id}`, orden_id: orden.id, no_orden: orden.no_orden || orden.id, cliente: orden.nombre, monto: Number(orden.total || 0), metodo: String(orden.metodo_pago || 'EFECTIVO').toUpperCase(), created_at: orden.created_at })
    }
    return cobros
...
async function obtenerResumenCierre() {
  ...
  const abonosCxc = obtenerAbonosTurno(cuentasRes.success ? cuentasRes.data || [] : [], turno.id, turno.created_at)
  const cobrosTaller = obtenerCobrosTaller(tallerRes.success ? tallerRes.data || [] : [], turno.id, turno.created_at)
