...
const abonoEditado = ref({ monto: 0, fecha: '', hora: '', metodo: '', nota: '' })
...
function buildPdfHtml(logoEmpresa = ''): string {
  ...
  const rowsPagos = pagos.value.map((p: any, index: number) =>
    `<tr><td>${index + 1}</td><td>${escapeHtml(p.fecha || '')} ${escapeHtml(p.hora || '')}</td><td>${escapeHtml(p.metodo || 'ABONO')}</td><td>${escapeHtml(p.nota || '')}</td><td class="right">${formatMoney(montoPago(p))}</td></tr>`
  ...
  <div class="w-full p-4 sm:p-6 space-y-5">
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <section class="rounded-2xl border border-surface-200 dark:border-surface-700 bg-surface-0 dark:bg-surface-800 overflow-hidden"><div class="p-4 border-b border-surface-200 dark:border-surface-700"><h3 class="font-bold">Historial de abonos</h3></div><DataTable :value="pagos" size="small" scrollable responsiveLayout="scroll"><Column header="#"><template #body="{ index }">{{ index + 1 }}</template></Column><Column field="fecha" header="Fecha"><template #body="{ data }">{{ data.fecha }} {{ data.hora || '' }}</template></Column><Column field="metodo" header="Método" /><Column header="Monto"><template #body="{ data }"><span class="font-bold text-emerald-600">{{ formatMoney(montoPago(data)) }}</span></template></Column><Column header="Acciones" style="width: 7rem"><template #body="{ data, index }"><div class="flex gap-1"><Button icon="pi pi-pencil" severity="info" text rounded size="small" @click="abrirEditarAbono(data, index)" v-tooltip="'Editar abono'" /><Button icon="pi pi-trash" severity="danger" text rounded size="small" @click="confirmarEliminarAbono(index)" v-tooltip="'Eliminar abono'" /></div></template></Column><template #empty><div class="text-center py-6 text-surface-500">Sin abonos registrados.</div></template></DataTable></section>
      </div>
